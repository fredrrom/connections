...
from pycop.parity.run_matrix_parity import MatrixParityCase, matrix_parity_row
from pycop.parity.run_status_check import (
    ReferenceMode,
    ReferenceProver,
    StatusCheckCase,
    status_check_row,
)
from pycop.parity.run_trace_parity import trace_parity_row
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Parity manifest JSON file. Defaults to the bundled manifests/full-0.1.json.",
    )
...
def _parity_script(kind: SweepKind) -> str:
    if kind == "status_check":
        return "python -m pycop.parity.run_status_check"
    return f"python -m pycop.parity.run_{kind}_parity"
...
def _diagnose_command(
    ...
    *,
    ...
    command = [
        ...
        "connections",
        "python",
        "python -m pycop.parity.diagnose_search",
        str(problem_path),
        "--logic",
        sweep.logic,
        "--domain",
