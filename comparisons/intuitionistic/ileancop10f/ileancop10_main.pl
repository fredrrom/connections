%% File: ileancop10_main.pl  -  Version: 1.0  -  Date: 5 Dec 2022
%%
%% Purpose: Call the IleanCoP core prover for a given formula
%%
%% Author:  Jens Otten
...
:- \+ prolog(eclipse) -> true ;
   ...
   [def_mm_intu], [ileancop10f].  % load leanCoP core prover
...
:- prolog(eclipse) -> true ;
   ...
   [def_mm_intu], [ileancop10f_swi].  % load nanoCoP core prover
...
ileancop10_main(File,Settings,Result) :-
    ...
    ( prove2(Problem2,Settings) ->
      ( Conj\=[] -> Result='Intuitionistic Theorem' ;
                    Result='Intuitionistically Unsatisfiable' ) ;
      ( Conj\=[] -> Result='Intuitionistic Non-Theorem' ;
                    Result='Intuitionistically Satisfiable' )
    ),
